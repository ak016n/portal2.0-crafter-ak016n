 ...
 * 
 * Takes care of making sure that a <code>SessionUser</code> gets
 * put into the SecurityContext Auth object (instead of just a String). 
 * <br/>
 * It also re-pulls the User from the UserService to make sure we have the very latest 
 * information (primarily GrantedAuthorities).  This is questionable to go back to the database
 * and defeats part of the point of using simple JWT tokens to handle User information.  At the very
 * least we probably want to pull the User from some sort of Cache.   
 * 
 * 
 * @author so1234
 *
